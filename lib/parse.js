...
var readLine = function (line) {
	if (line.indexOf('\t') === 0) {
		...
		if (line.indexOf('\tname ') >= 0 || line.indexOf('\tName:') >= 0) {
			// Check to add the current entry to the databse.
			if (this.currentEntry) {
				...
				this.database.push(this.currentEntry)
			}
...
var readClose = function () {
	// Depict the first entry as the header.
	if (this.options.hasHeader) {
		if (this.database[0]) {
			if (this.database[0].entries) {
				delete this.database[0].entries
			}
			this.database[0].isHeader = true
		}
	}
...
var parse = function (input, options) {
	// Prepare the options.
	options = options || {}
	options.hasHeader = options.hasHeader || true
