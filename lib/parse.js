...
const getLineDetail = function (line) {
	...
	if (trimmedLine.length > 3 && trimmedLine.charAt(trimmedLine.length - 1) === ')') {
		...
		if (match) {
			...
			// Retrieve the crc if it's there.
			...
			// Retrieve the sha1 if it's there.
			const sha1 = entry.match(/ sha1 (\S*) /i)
			if (sha1) {
				output.sha1 = sha1[1]
			}

			// Retrieve the serial if it's there.
			let serial = entry.match(/ serial "([^"]*)" /i)
			if (serial) {
				output.serial = serial[1]
			} else {
				serial = entry.match(/ serial (\S*) /i)
				if (serial) {
					output.serial = serial[1]
				}
			}
...
const isEntryLine = function (line) {
	const trimmedLine = line.trim()
	return trimmedLine.indexOf('rom (') === 0 || trimmedLine.indexOf('file (') === 0
}

const hasCompleteEntry = function (line) {
	const trimmedLine = line.trim()
	return trimmedLine.charAt(trimmedLine.length - 1) === ')'
}

const getBufferedEntryDetail = function (line) {
	return getLineDetail(line.replace(/\s+/g, ' '))
}

const readLine = function (line) {
	if (this.pendingEntryLine) {
		this.pendingEntryLine += ` ${line.trim()}`
		if (!hasCompleteEntry(this.pendingEntryLine)) {
			return
		}

		const details = getBufferedEntryDetail(this.pendingEntryLine)
		delete details.lineType
		this.currentEntry.entries.push(details)
		this.pendingEntryLine = null
		return
	}
	...
	if (line.indexOf('\t') === 0 || line.substring(0, 2) === '  ') {
		...
		if (line.includes('\tname ') || line.includes('  name ') || line.includes('\tName:') || line.includes('  Name:')) {
			...
			if (isEntryLine(line) && !hasCompleteEntry(line)) {
				this.pendingEntryLine = line.trim()
				return
			}
...
const readError = function (error) {
...
const parse = function (input, options) {
	...
	return new Promise((resolve, reject) => {
		...
		lineReader.pendingEntryLine = null
